...
> **Toolchain (ADR 0019).** The original plan used mdBook (dev) + VitePress (user) + redoc
> (API). This was superseded by a single **Fumadocs** site that covers all three surfaces.
> The Next.js docs app lives in `docs/site/` (isolated package); MDX content stays in
> `docs/dev/` and `docs/user/` as below. The site builds fully static (`output: export`,
> `basePath: /shalgalt-omr`) and deploys to the `gh-pages` branch root with
> `keep_files: true`, coexisting with the updater `latest.json` (ADR 0018). See
> `.github/workflows/docs.yml`. The site is **multilingual** (Fumadocs i18n): default
> locale `mn` (Mongolian), secondary `en` (English). Developer docs are authored in English
> (unsuffixed files, English under both locales per the repo's English-docs rule); the user
> manual is authored in Mongolian (unsuffixed) with progressive `*.en.mdx` translations.

Two audience trees, one site:

### 12.1 Developer documentation (`docs/dev/`, English)
...
- ADR index
- HTTP API reference — static MDX generated by `fumadocs-openapi` from the `openapi.json`
  that `shalgalt-core` produces (`cargo run -p shalgalt-core --example dump_openapi`), so
  it cannot drift from the router.

### 12.2 User documentation (`docs/user/`, in Mongolian)
...
- Sample `.shalgalt` projects (`docs/user/samples/`, three variants: open / encrypted /
  with PDF) generated by `cargo run -p shalgalt-fileformat --example generate_samples`,
  plus walkthrough storyboards under `docs/user/walkthroughs/`.

Per-release PDF exports of both manuals are deferred to a later docs iteration (ADR 0019
follow-up); the static HTML site is the v1.0 deliverable. Screenshot capture is tracked in
`docs/screenshots-needed.md`.
