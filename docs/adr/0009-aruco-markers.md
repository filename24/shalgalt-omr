...
| ArUco DICT_4X4_50 | per-marker | minimal — only 16-bit data | ~6 × 6 mm (small) | OpenCV `aruco::detectMarkers` |
| **ArUco DICT_6X6_50** | per-marker | one-bit Hamming distance ≥ 4 between dictionary entries | ~8 × 8 mm | OpenCV `aruco::detectMarkers` |
| ArUco DICT_7X7_1000 | per-marker | strong | ~10 × 10 mm | OpenCV `aruco::detectMarkers` |
...
- **DICT_6X6_50** is a sweet spot. The 6×6 inner data grid plus the mandatory
