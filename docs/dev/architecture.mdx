---
title: Architecture
description: Layers, data flow, and the four hard rules that every PR must respect.
---
...
### Rule 1 — IPC carries path strings, never bytes
...
reported by emitting `task-progress` events with snake_case stage names
(`TaskProgress` / `TaskStage` in `shalgalt-core::domain::progress`). The command thread
returns immediately.

### Rule 3 — One template, one JSON column
...
### Rule 4 — The axum router never owns its lifecycle
